# School Accountability Models Plots
...
AF_grades_metrics <- read_csv("data/AF_grades_metrics_by_subgroup.csv")
AF_grades_final <- read_csv("data/AF_grades_final.csv")

AF_grades_metrics %>%
    filter(subgroup == "All Students") %>%
    full_join(AF_grades_final, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    ggplot(aes(x = final_grade, y = success_rate)) + 
...
AF_grades_metrics %>%
    filter(!is.na(tvaas_composite)) %>%
    filter(subgroup == "All Students") %>%
    inner_join(AF_grades_final, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    with(table(final_grade, tvaas_composite))

AF_grades_metrics %>%
    filter(!is.na(tvaas_composite)) %>%
    filter(subgroup == "All Students") %>%
    with(table(grade_relative_achievement))
...
AF_grades_final %>%
    ...
    geom_jitter(position = position_jitter(width = .4, height = 0)) +
    theme_hc() +
    scale_y_continuous(limits = c(0, 50), breaks = 10 * (0:10)) +
    xlab("Final Grade") +
    ylab("Percent SWD")
