...
grade_bands <- school_assessment %>%
    ...
    select(year, system, system_name, school, school_name, test, subject, subgroup,
...
combined_grades <- grade_bands %>%
    ...
    transmute(
        year, system, system_name, school, school_name, test, subject, subgroup,
        valid_tests, n_below, n_approaching, n_on_track, n_mastered,
        pct_below, pct_approaching, pct_on_track, pct_mastered, pct_on_mastered
    ) %>%
    arrange(subject, subgroup)

write_csv(combined_grades, "N:/ORP_accountability/data/2019_final_accountability_files/school_release_file.csv", na = "")

combined_grades %>%
    suppress(threshold = 5) %>%
    write_csv("N:/ORP_accountability/data/2019_final_accountability_files/school_release_file_suppressed.csv", na = "")
