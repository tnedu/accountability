...
state_assessment <- read_csv("N:/ORP_accountability/data/2018_final_accountability_files/state_assessment_file.csv") %>%
    filter(year == 2018)

grade_bands <- state_assessment %>%
    filter(
        grade != "All Grades",
...
combined_grades <- grade_bands %>%
    ...
    transmute(year, system, system_name, test, subject, subgroup,
        valid_tests, n_below, n_approaching, n_on_track, n_mastered,
        pct_below, pct_approaching, pct_on_track, pct_mastered, pct_on_mastered
    ) %>%
    arrange(subject, subgroup)

write_csv(combined_grades, "N:/ORP_accountability/data/2018_final_accountability_files/state_release_file.csv", na = "")

combined_grades %>%
    suppress() %>%
    write_csv("N:/ORP_accountability/data/2018_final_accountability_files/state_release_file_suppressed.csv", na = "")

state_assessment %>%
    suppress() %>%
    write_csv("N:/ORP_accountability/data/2018_final_accountability_files/state_assessment_file_suppressed.csv", na = "")
