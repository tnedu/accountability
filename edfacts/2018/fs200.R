...
fs200 <- school_grades %>%
    ...
    transmute(first = 1:nrow(school_grades),
        ...
        indicator_status = case_when(
            is.na(score_achievement) ~ "TOOFEW",
            !is.na(score_achievement) ~ "STTDEF"
        ),
            state_defined_status = score_achievement
    )

write_csv(fs200, "N:/ORP_accountability/projects/Alex/accountability/edfacts/2018/TNSCHACHIVSTATv1.csv", na = "", col_names = FALSE)

paste("SCH ACAD ACHIEVE STATUS", nrow(fs200), "TNSCHACHIVSTATv1.csv", "TNSCHACHIVSTATv1", "2017-2018", "", sep = ",")
