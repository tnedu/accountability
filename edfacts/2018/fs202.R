...
fs202 <- school_grades %>%
    ...
    transmute(first = 1:nrow(school_grades),
        ...
        explanation = "",
        indicator_type = "IND01",
        indicator_status = case_when(
            is.na(score_absenteeism) ~ "TOOFEW",
            !is.na(score_absenteeism) ~ "STTDEF"
        ),
        state_defined_status = score_absenteeism
    )

write_csv(fs202, "N:/ORP_accountability/projects/Alex/accountability/edfacts/2018/TNSCHQUALSTATv1.csv", na = "", col_names = FALSE)

paste("SCH QUAL STUD SUC", nrow(fs202), "TNSCHQUALSTATv1.csv", "TNSCHQUALSTATv1", "2017-2018", "", sep = ",")
