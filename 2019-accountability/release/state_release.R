...
grade_bands <- state_assessment %>%
    ...
    group_by(year, test, subject, subgroup) %>%
    ...
    select(year, test, subject, subgroup, 
...
combined_grades <- grade_bands %>%
    ...
    transmute(
        year, test, subject, subgroup, 
        valid_tests, n_below, n_approaching, n_on_track, n_mastered,
        pct_below, pct_approaching, pct_on_track, pct_mastered, pct_on_mastered
    ) %>%
    arrange(subject, subgroup)

write_csv(combined_grades, "N:/ORP_accountability/data/2019_final_accountability_files/state_release_file.csv", na = "")

combined_grades %>%
    suppress() %>%
    write_csv("N:/ORP_accountability/data/2019_final_accountability_files/state_release_file_suppressed.csv", na = "")
