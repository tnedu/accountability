...
ACT_prior <- read_dta("K:/ORP_accountability/data/2015_ACT/ACT_district2015.dta") %>%
    ...
    transmute(year = 2014, system, system_name, subject = "ACT Composite",
...
ACT <- read_dta("K:/ORP_accountability/data/2015_ACT/ACT_district2016.dta") %>%
    filter(subgroup %in% c("All Students", "Black/Hispanic/Native American", "Economically Disadvantaged",
        "Students with Disabilities", "English Language Learners with T1/T2", "Super Subgroup")) %>%
    transmute(year = 2015, system, system_name, subject = "ACT Composite",
        subgroup = ifelse(subgroup == "English Language Learners with T1/T2", "English Learners", subgroup),
        valid_tests = valid_tests_nogradcohort, n_prof = num_21_orhigher_nogradcohort)
...
percent_PA_BB <- system_base %>%
    ...
    # Percent PA and Upper Bound CI
        pct_prof_adv = round((n_prof + n_adv)/valid_tests, 3),
    ...
    select(year, system, system_name, subject, subgroup, valid_tests, pct_below_bsc, lower_bound_ci_BB, pct_prof_adv, upper_bound_ci_PA)
