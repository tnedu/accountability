...
/***************************************************************
Do File description:  2016 ACT Substitution for District Accountability
...
mmerge unique_student_id using `act_student_math', type(n:1) umatch(state_stud_id);
...
mmerge unique_student_id using `act_student_read', type(n:1) umatch(state_stud_id);
keep if _merge == 2 & grade == 11 & act_read != .;

keep unique_student_id last_name first_name middle_i grade system school act_read;
