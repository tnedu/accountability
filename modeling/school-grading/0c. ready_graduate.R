...
ACT_grad <- school_accountability %>%
    ...
    transmute(system, school, subgroup,
        ...
        ACT_grad_prior = round(grad_rate_prior * pct_prof_adv_prior/100 + 1e-10, 1),
        ACT_grad = round(grad_rate * pct_prof_adv/100 + 1e-10, 1),
        AMO_target = round(ACT_grad_prior + (100 - ACT_grad_prior)/16 + 1e-10, 1),
        AMO_target_4 = round(ACT_grad_prior + (100 - ACT_grad_prior)/8 + 1e-10, 1),
