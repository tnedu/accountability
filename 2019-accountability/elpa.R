...
wida <- read_csv("N:/Assessment_Data Returns/ACCESS for ELs and ALT/2018-19/TN_Summative_StudRR_File_06032019.csv", guess_max = 20000) %>%
    clean_names() %>%
    filter(
        reported_mode != "Mixed",
...
# Deduplicate by keeping highest composite scale score, then highest literacy scale score
    group_by(student_id) %>%
    mutate(max_comp = max(prof_composite, na.rm = TRUE)) %>%
    filter(prof_composite == max_comp) %>%
    mutate(max_lit = max(prof_literacy, na.rm = TRUE)) %>%
    filter(prof_literacy == max_lit) %>%
    ungroup() %>%
# Deduplicate by non-missing race/ethnicity and grade
    ...
    mutate(
        ...
        growth_standard_denom = not_na(prof_composite) & not_na(prof_composite_17),
