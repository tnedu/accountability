...
global SAS_base "K:\ORP_accountability\data\2015_sas_accountability\ASD included grades/school_base_2015_19jul2015.csv";
global fall_preview "K:\ORP_accountability\projects\2015_fall_district_preview\Alex\Output/school_base_with_super_subgroup_apr5.csv";
...
* Homebound students do not appear in school level files;
drop if homebound == 1;
drop if residential_facility == 1;

* Drop historical data for schools moving to ASD;
drop if system == 792 & (school == 2150 | school == 2208 | school == 2820 | school == 2425 | school == 2765);
...
quietly foreach s in All Asian Black Hispanic Hawaiian Native White BHN ED SWD ELL ELL_T1_T2 Non_ED Non_SWD Non_ELL Non_ELL_T1_T2 Super {;
	...
	* All Grades;
	...
	collapse (sum) enrolled tested valid_test n_below_bsc n_bsc n_prof n_adv, by(year system system_name school school_name original_subject subgroup);
	...
	* Individual Grades;
	...
	collapse (sum) enrolled tested valid_test n_below_bsc n_bsc n_prof n_adv, by(year system system_name school school_name original_subject grade subgroup);
...
order year, first;
order system_name school school_name subject grade subgroup, after(system);
...
foreach y in 2015 2016 {;
	...
	keep year system system_name school school_name subject grade subgroup enrolled tested valid_tests n_21_and_above pct_21_and_above n_below_19 pct_below_19;
...
foreach y in 2014 2015 2016 {;
	...
	use $grad/school_grad_rate`y'.dta, clear;
	...
	keep year system system_name school school_name subject grade subgroup grad_cohort grad_count grad_rate dropout_count dropout_rate;
...
reshape wide enrolled tested valid_tests n_below_bsc n_bsc n_prof n_adv 
	...
	i(year system system_name school school_name subject grade) j(subgroup) string;
...
keep year system system_name school school_name subject grade subgroup enrolled tested valid_tests 
...
* Drop entries for schools with only prior year data;
bysort system school: egen temp = max(year);
drop if temp == `lag_yr';
drop temp;

* Clean and output base file;
replace system_name = "Shelby County" if system_name == "Shelby County (New)";

gsort system system_name school school_name subject grade subgroup -year;
...
save "$output/school_base_with_super_subgroup_2016.dta", replace;
export delim using "$output/school_base_with_super_subgroup_2016.csv", delim(",") replace;
