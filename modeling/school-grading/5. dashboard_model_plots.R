...
AF_grades_metrics_2 <- read_csv("data/AF_bottom_five_amos_metrics_alternate_weights.csv")
AF_grades_final_2 <- read_csv("data/AF_bottom_five_amos_final_grades_alternate_weights.csv")

school_accountability %>%
    filter(subgroup == "All Students" & subject == "Success Rate") %>%
    full_join(AF_grades_final_2, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    ggplot(aes(x = final_grade, y = pct_prof_adv)) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(position = position_jitter(width = .4, height = 0)) +
    theme_hc() +
...
AF_grades_metrics_2 %>%
    filter(!is.na(grade_tvaas)) %>%
    filter(subgroup == "All Students") %>%
    inner_join(AF_grades_final_2, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    with(table(final_grade, grade_tvaas))
...
readstata13::read.dta13("K:/ORP_accountability/projects/2015_school_coding/Output/reward_2015_ap.dta") %>%
    filter(reward == 1) %>%
    select(system, school, reward_performance, reward_progress) %>%
    full_join(AF_grades_final_2, by = c("system", "school")) %>%
    mutate(reward_performance = ifelse(is.na(reward_performance), 0, reward_performance)) %>%
    mutate(reward_progress = ifelse(is.na(reward_progress), 0, reward_progress)) %>%
    with(table(reward_progress, final_grade))
...
readstata13::read.dta13("K:/ORP_accountability/projects/2015_school_coding/Output/priority_schools_not_exiting_ap.dta") %>%
    filter(priority == 1) %>%
    select(system, school, priority) %>%
    left_join(AF_grades_final_2, by = c("system", "school")) %>%
    mutate(priority = ifelse(is.na(priority), 0, priority)) %>%
    with(table(priority, final_grade), useNA = "ifany")

# Weighting option 3

AF_grades_metrics_3 <- read_csv("data/AF_bottom_five_amos_metrics_alternate_weights_2.csv")
AF_grades_final_3 <- read_csv("data/AF_bottom_five_amos_final_grades_alternate_weights_2.csv")

school_accountability %>%
    filter(subgroup == "All Students" & subject == "Success Rate") %>%
    full_join(AF_grades_final_3, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    ggplot(aes(x = final_grade, y = pct_prof_adv)) +
    geom_boxplot(outlier.shape = NA) +
    geom_jitter(position = position_jitter(width = .4, height = 0)) +
    theme_hc() +
...
AF_grades_metrics_3 %>%
    filter(!is.na(grade_tvaas)) %>%
    filter(subgroup == "All Students") %>%
    inner_join(AF_grades_final_3, by = c("system", "system_name", "school", "school_name", "designation_ineligible", "pool")) %>%
    with(table(final_grade, grade_tvaas))
...
readstata13::read.dta13("K:/ORP_accountability/projects/2015_school_coding/Output/reward_2015_ap.dta") %>%
    filter(reward == 1) %>%
    select(system, school, reward_performance, reward_progress) %>%
    full_join(AF_grades_final_3, by = c("system", "school")) %>%
    mutate(reward_performance = ifelse(is.na(reward_performance), 0, reward_performance)) %>%
    mutate(reward_progress = ifelse(is.na(reward_progress), 0, reward_progress)) %>%
    with(table(reward_progress, final_grade))
...
readstata13::read.dta13("K:/ORP_accountability/projects/2015_school_coding/Output/priority_schools_not_exiting_ap.dta") %>%
    filter(priority == 1) %>%
    select(system, school, priority) %>%
    left_join(AF_grades_final_3, by = c("system", "school")) %>%
    mutate(priority = ifelse(is.na(priority), 0, priority)) %>%
    with(table(priority, final_grade), useNA = "ifany")
