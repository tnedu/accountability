...
global tvaas "K:\ORP_accountability\data\2015_tvaas/School-Level Intra-Year NCE MRM and URM Results (All Students).xlsx";
global tvaas_subgroup "K:\ORP_accountability\data\2015_tvaas/School-Level Intra-Year NCE MRM and URM Results (Subgroups).xlsx";
...
collapse (sum) enrolled tested valid_tests n_below_bsc n_bsc n_prof n_adv, 
	by(year system system_name school school_name subject grade subgroup);
...
collapse (sum) enrolled tested valid_tests n_below_bsc n_bsc n_prof n_adv,
	by(year system system_name school school_name subject subgroup grade_band);
...
duplicates tag year system school subject grade comparison, gen(dup);
...
foreach y in 1 2 {;
	...
	gen tested_lag_`y'yr = tested[_n+`y']
		if system == system[_n+`y'] & school == school[_n+`y'] & subject == subject[_n+`y'] & grade == grade[_n+`y'] & subgroup == subgroup[_n+`y'];
...
destring systemnumber schoolnumber year index, replace;
...
drop system school;
rename (systemnumber schoolnumber index level) (system school tvaas_index tvaas_level);

drop if test == "ACT" & subject != "ACT Composite";

keep year system school subject grade tvaas_index tvaas_level;
...
keep year system school subject subgroup grade tvaas_index tvaas_level;
...
mmerge year system school subject grade subgroup using `tvaas_all', type(1:1);
drop if _merge == 2;
drop _merge;

mmerge year system school subject grade subgroup using `tvaas_subgroup', type(1:1) update;
drop if _merge == 2;
drop _merge;
...
export delim using "$output/school_numeric_with_super_subgroup_2016.csv", delim(",") replace;
